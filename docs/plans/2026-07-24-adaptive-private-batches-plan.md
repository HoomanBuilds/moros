...
4. Cache every browser order proving artifact, then make the first order flow relay an idempotent epoch-open call that persists its 60-second cutoff before proof generation.
...
10. Cache generated batch proofs and retry transient RPC submission failures without proving again.
11. Update frontend waiting logic, market copy, countdowns, privacy disclosure, and portfolio states.
12. Extend unit and lifecycle tests for singleton, one-sided partial, mixed partial, full, stale state, proof failure, timeout refund, resolution, claim, and LP harvest.
13. Build fresh proving artifacts and contract WASM files.
14. Recover safely accessible testnet funds without importing any old market data.
15. Erase Moros Supabase records and objects, browser namespaces, and VM runtime registries.
16. Deploy a fresh standard-named contract stack and write one canonical deployment manifest.
17. Repackage and activate the private service and keeper with the fresh manifest.
18. Verify exact IDs through public config, VM environment, frontend configuration, and direct contract reads.
19. Run a completely fresh live lifecycle.
20. Run final Rust, circuit, service, TypeScript, unit, lint, build, browser, security, dependency, stale-reference, and storage checks.
21. Commit and push the verified feature branch without merging `main`.
...
- A transient RPC failure retries the same cached batch proof and cannot force proof regeneration.
