...
const riskGroups = reflectorRiskGroups(network.id);
const ENV_PREFIX = `MOROS_${network.id.toUpperCase()}_`;
const scoped = (suffix) => process.env[`${ENV_PREFIX}${suffix}`] || "";
const legacy = (suffix) =>
  network.id === "testnet" ? process.env[`MOROS_${suffix}`] || "" : "";
...
const DEPLOYMENT_NAME =
  scoped("DEPLOYMENT_NAME") ||
  legacy("DEPLOYMENT_NAME") ||
  `Moros ${network.id === "mainnet" ? "Mainnet" : "Testnet"}`;
const SALT_NAMESPACE =
  scoped("DEPLOYMENT_SALT") ||
  legacy("DEPLOYMENT_SALT") ||
  `moros-${network.id}-release`;
const COLLATERAL = network.collateralContract;
const STATE_PATH = resolve(
  cfg.repo,
  scoped("DEPLOYMENT_STATE") ||
    legacy("DEPLOYMENT_STATE") ||
    `deployments/private-${network.id}.local.json`,
);
...
const BUILD_ROOT = resolve(
  resolve(cfg.repo, network.artifactPath, ".."),
);
...
const ROUNDING_RESERVE = BigInt(
  scoped("ROUNDING_RESERVE") ||
    legacy("ROUNDING_RESERVE") ||
    "10000000",
);
const Q32 = 1n << 32n;
...
async function main() {
  if (cfg.network !== network.id) {
    throw new Error("service network selection is inconsistent");
  }
  if (!SECRET) {
    throw new Error(
      `MOROS_${network.id.toUpperCase()}_DEPLOYER_SK is required`,
    );
  }
  if (!PRIVACY_SECRET) {
    throw new Error(
      `MOROS_${network.id.toUpperCase()}_PRIVACY_SK is required`,
    );
  }
  if (
    network.id === "mainnet" &&
    !/^[0-9a-f]{40}$/u.test(SOURCE_COMMIT)
  ) {
    throw new Error("MOROS_SOURCE_COMMIT is required for mainnet");
  }
  ...
  if (
    ...
    throw new Error(`invalid private ${network.id} proving manifest`);
  }
  ...
  const identity = networkPrivacyIdentity(PRIVACY_SECRET, network.id);
  const salts = Object.fromEntries(
    [
      ...
      name,
      deterministicSalt(`${SALT_NAMESPACE}:${name}`, network.id),
    ]),
  );
  ...
  const sameDeployment =
    previousState.network === network.id &&
  ...
  await deployContract({
    ...
    salt: salts.resolver,
    args: {
      oracles: [reflector.cexOracle, reflector.fiatOracle],
      quorum: 1,
      max_age: 3_600n,
  ...
  await deployContract({
    ...
    args: {
      ...
      initial_routes: reflectorResolverRoutes(ids.resolver, network.id),
    },
  });
  ...
  if (
    ...
    registryRoutes.length !== riskGroups.length ||
    registryRoutes.some((route, index) =>
      route.resolver !== ids.resolver ||
      route.risk_group !== riskGroups[index].risk_group ||
  ...
  const publicDeployment = {
    ...
    privacy: {
      ...
      testnetSingleVmCommittee: network.id === "testnet",
    },
    marketPolicy: {
      allowedAssets: reflector.freeAssets,
      assetRiskGroups: riskGroups,
      liquidityTiers: [
        200_000_000n,
        500_000_000n,
        1_000_000_000n,
    ...
    mainnetReady: network.mainnetReady,
  };
  savePublic(publicDeployment);
  saveState({ ...state, complete: true, publicDeployment });
  process.stdout.write(
    `private ${network.id} contracts ready: factory ${ids.factory}, vault ${ids.sharedVault}, pool ${ids.liquidityPool}\n`,
  );
}
