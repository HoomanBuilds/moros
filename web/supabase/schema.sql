...
create policy "comments_insert_owner" on comments for insert with check ((auth.jwt() -> 'app_metadata' ->> 'wallet') = wallet);
create policy "comments_update_owner" on comments for update using ((auth.jwt() -> 'app_metadata' ->> 'wallet') = wallet);
