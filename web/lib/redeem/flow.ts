...
export async function runRedeem(
  ...
  { position: { amount: string; stakeAmount?: string; side: string; secret: string; nullifier: string; commitment: string }; address: string; marketId: string; poolId: string; onStage: (s: RedeemStage) => void }
) {
  onStage("preparing");
  ...
  if (outcome === "VOID") throw new Error("voided positions use the full refund action");
  ...
  onStage("submitting");
  const res = await postRedeem({ proof, publicSignals, recipient: address, poolId });
  onStage("done");
  const txHash = res && typeof res === "object" && "txHash" in res && typeof res.txHash === "string"
    ? res.txHash
    : undefined;
  return { txHash };
}
